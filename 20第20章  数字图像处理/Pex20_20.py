...
n = len(f);
a = np.array(Image.open(f[0]))
a = a.astype(float);
jj = np.arange(1, n)
L1 = a[:, 0];
L2 = a[:, -1]  # 拼接的大图像左边界和右边界初始化
...
for i in range(n - 1):
    ...
    for j in jj:
        ...
        cha2 = abs(L2 - e1)._sum()
        tcha1.append(cha1);
        tcha2.append(cha2)  # 左右边界的差异
    m1 = np.array(tcha1).min();
    m2 = np.array(tcha2).min()

    if abs(L1 - 255.)._sum() < 1:
